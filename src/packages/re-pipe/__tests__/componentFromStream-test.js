...
import { map, mapTo, filter, first, last, startWith, tap } from 'rxjs/operators'
...
test('handler multiple observers of props stream', () => {
  ...
  const Div = componentFromStream(props$ =>
    ...
    combineLatest(props$, props$, props$).pipe(
      map(([props1]) => <Other {...props1} />)
    )
  )
...
test('complete props stream before unmounting', () => {
  ...
  const Div = componentFromStream(props$ => {
    ...
    return combineLatest(props$, first$, last$).pipe(
      map(([props1]) => <div {...props1} />)
    )
  })
...
test('completed props stream not should throw an exception', () => {
  const Div = componentFromStream(props$ => {
    const first$ = props$.pipe(filter(() => false), first(), startWith(null))

    return combineLatest(props$, first$).pipe(
      map(([props1]) => <div {...props1} />)
    )
  })
